...
static const uint8_t spp_adv_data[23] = {
    0x02,0x01,0x06,
    0x03,0x03,0xF0,0xAB,
    0x0F,0x09,0x45,0x2D,0x50,0x55,0x43,0x4B,0x32,0x5F,0x42,0x4C,0x45,0x53,0x50,0x50
};
...
void send_indicate(uint8_t *data, uint8_t size) {
	if(!enable_data_ntf){
		ESP_LOGE(GATTS_TABLE_TAG, "%s do not enable data Notify\n", __func__);
		return;
	}
	esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL], size, data, false);
}
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
					...
					uart_set_motors_state((uint8_t *)(p_data->write.value));
#endif
                }else{
                    //TODO:
                }

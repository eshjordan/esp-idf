...
static int show_socket_error_reason(const char *str, int socket)
{
    ...
    if (err != 0) { ESP_LOGW(TAG, "%s socket error %d %s", str, err, strerror(err)); }
...
void inter_robot_comms_task(void *pvParameter)
{
    ...
    uint8_t actuators_buff[ACTUATORS_BUFF_LEN],
        actuators_buff_last[ACTUATORS_BUFF_LEN]; // Packet id (1) + behavior/others (1) + speed left (2) + speed right
                                                 // (2) + LEDs (1) + RGB LEDs (12) + sound (1)
    uint8_t header[1];
    ...
    while (1)
    {
        ...
        case 3: // Receive commands (new actuators values).
            ...
            if (rx_bytes_not_read >= ACTUATORS_BUFF_LEN * 5)
            { // Flush the input buffer to keep the robot responsive to recent commands.
                // printf("flushing rx...\r\n");
                num_cmd_packets = rx_bytes_not_read / ACTUATORS_BUFF_LEN;
                if ((num_cmd_packets * ACTUATORS_BUFF_LEN) == rx_bytes_not_read)
                { // Keep the last command valid.
                    for (int i = 0; i < (num_cmd_packets - 1); i++)
                    {
                        recv(client_sock, &actuators_buff[0], ACTUATORS_BUFF_LEN, MSG_DONTWAIT);
                    }
                } else
                { // The new command is partially received, so keep it and remove all the previous ones.
                    // printf("b");
            ...
            if (len < 0)
            {
                if (get_socket_error_code(client_sock) != 11)
                { // When error code is 11, it means that no data was available, so continue normally with the next
                  // state, otherwise terminate the connection.
            ...
            if (valid_cmd_received == 1)
            { // Wait for at least the first command before sending back anything.
        ...
        case 4: // Exchanging image.
            ...
            header[0] = 0x01;
            if (send(client_sock, header, 1, 0) < 0)
            { // Send id=0x01
            ...
            {
                if ((len =
                         send(client_sock, &(img_buff->data[SPI_PACKET_MAX_SIZE * packet_id]), SPI_PACKET_MAX_SIZE, 0))
                    < 0)
                {
            ...
            if (remaining_bytes > 0 && conn_error == 0)
            { // If there is a last image segment and no errors occurred.
                if ((len = send(client_sock, &(img_buff->data[SPI_PACKET_MAX_SIZE * packet_id]), remaining_bytes, 0))
                    < 0)
                {
                    show_socket_error_reason("send_data", client_sock);
                    conn_error = 1;
...
void inter_robot_comms_set_event_connected(void) { xEventGroupSetBits(socket_event_group, CONNECTED_BIT); }

void inter_robot_comms_set_event_disconnected(void) { xEventGroupSetBits(socket_event_group, DISCONNECTED_BIT); }
